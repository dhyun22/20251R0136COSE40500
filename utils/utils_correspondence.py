...
def find_nearest_patchs(mask1, mask2, image1, image2, features1, features2, mask=False, resolution=None, edit_image=None):
    ...
    matched_coords_candidates = []
    h_feat, w_feat = features1.shape[2], features1.shape[3]  # 예: 60 x 60 patch

    # mask1/2를 patch 해상도에 맞게 다운샘플링
    mask1_patch = F.interpolate(mask1.unsqueeze(0).unsqueeze(0), size=(h_feat, w_feat), mode='nearest').squeeze(0).squeeze(0).bool()
    mask2_patch = F.interpolate(mask2.unsqueeze(0).unsqueeze(0), size=(h_feat, w_feat), mode='nearest').squeeze(0).squeeze(0).bool()

    for idx, tgt_idx_item in enumerate(nearest_patch_indices):
        y1, x1 = divmod(idx, w_feat)
        y2, x2 = divmod(tgt_idx_item.item(), w_feat) # .item()을 사용하여 텐서 스칼라를 파이썬 숫자로 변환

        # 둘 다 mask 영역 안에 있을 때만 추가
        if mask1_patch[y1, x1] and mask2_patch[y2, x2]:
            # 해당 대응점 쌍의 거리 값을 가져옵니다.
            # distances는 torch.cdist 결과로 (N_src, N_tgt) 형태이므로,
            # distances[idx, tgt_idx_item]으로 접근합니다.
            current_distance = distances[idx, tgt_idx_item].item() # .item()으로 파이썬 float 변환
            matched_coords_candidates.append((current_distance, (y1, x1), (y2, x2)))

    # 유사도(거리가 작을수록 유사도 높음)를 기준으로 정렬
    matched_coords_candidates.sort(key=lambda x: x[0]) # 거리를 기준으로 오름차순 정렬 (가장 작은 거리가 가장 유사)

    # breakpoint()
    top_k_to_append = 20 # 여기에 원하는 개수를 설정합니다.
    low_k_to_append = 20
    matched_coords = [] # 최종적으로 반환될 matched_coords 리스트
    for dist, (y1, x1), (y2, x2) in matched_coords_candidates[:top_k_to_append]:
        matched_coords.append(((y1, x1), (y2, x2)))
    for dist, (y1, x1), (y2, x2) in matched_coords_candidates[100:low_k_to_append+100]:
        matched_coords.append(((y1, x1), (y2, x2)))
    ...
    return nearest_patches_image, resized_image2, matched_coords
